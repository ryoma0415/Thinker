...
class SerialApp:
    def __init__(self, master):
        ...
        self.load_calibration_button = tk.Button(master, text="キャリブレーション　読み込み", font=button_font, command=self.load_calibration_command)
        self.load_calibration_button.pack(pady=10)
        self.load_calibration_button.place(x=10, y=440)

        # キャリブレーション用の計測ボタン作成
        button_place = [[670, 280],[475, 470],[920, 280],[1050, 470],[785, 690],[750, 280],[555, 470],[1000, 280],[1130, 470],[865, 690]]
        button_font_2 = ("Arial", 12)

        for i in range(10):
            button = tk.Button(master, text=f"記録 {i+1}", font=button_font_2, command=lambda idx=i: self.get_sensor_command(idx))
            button.pack(pady=5)
    ...
    def load_calibration_command(self):
        try:
            with open('TK-01_Calibration.pkl', 'rb') as f:
                self.calibration_data = pickle.load(f)
        except FileNotFoundError:
            messagebox.showerror("Error", "キャリブレーションファイルが見つかりません。")
            return
        except Exception as e:
            messagebox.showerror("Error",f"An error occurred while loading the calibration data: {e}")
            return
